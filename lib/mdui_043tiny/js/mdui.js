...
;(function(global, factory) {
    ...
    $.extend({
      ...
      ajax: function ajax(options) {
        ...
        var defaults = {
          ...
          // 请求的数据，查询자符串或对象
          data: false,
          // 是否把数据转换为查询자符串发送，为 false 时不进行自动转换。
          processData: true,
          // 是否为异步请求
          async: true,
        ...
        // 需要发送的数据
        // GET/HEAD 请求和 processData 为 true 时，转换为查询자符串格式，特殊格式不转换
        var sendData;
        if (
  ...
  (function () {
     ...
     *
     * 1. 代理组件 API 执行初始化函数, selector 必须为자符串.
  ...
  mdui.Headroom = (function () {
     ...
     */
    var DEFAULT = {
      tolerance: 5,                                 // 滚动条滚动多少距离开始隐藏或显示元素，{down: num, up: num}，或数자
  ...
  mdui.Fab = (function () {
    ...
    /**
     * 浮动操作按钮实例
     * @param selector 选择器或 HTML 자符串或 DOM 元素或 JQ 对象
     * @param opts
     * @constructor
     */
  ...
  mdui.Drawer = (function () {
    ...
    /**
     * 抽屉栏实例
     * @param selector 选择器或 HTML 자符串或 DOM 元素
     * @param opts
     * @constructor
     */
  ...
  mdui.prompt = function (label, title, onConfirm, onCancel, options) {
    ...
    return mdui.dialog({
      ...
      onOpen: function (inst) {
        ...
        // 有자符数限制时，加载完文本框后 DOM 会变化，需要更新对话框高度
